...
class MyGrid(gamegridp.PixelGrid , gamegridp.GUIGrid):
    """My Grid with custom setup method."""
    def setup(self):
        self.toolbar.add_button("Start Rocket", "images/ship.png", color=(200,200,200), border=(50,50,50))
        self.rocket = Rocket(grid = self, cell=(80,180))
        self.set_background("images/galaxy.jpg")
        self.run()
...
class Rocket(gamegridp.Actor):
    ...
    def act(self):
        if not self.grid.is_valid_move(self.look_forward(), self):
            self.remove()
        elif self.started:
            self.move()
